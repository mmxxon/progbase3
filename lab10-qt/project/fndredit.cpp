...
FndrEdit::FndrEdit(QWidget *parent) : QDialog(parent), ui(new Ui::FndrEdit) {
  ui->setupUi(this);
}

FndrEdit::~FndrEdit() { delete ui; }
void FndrEdit::on_okay_button_clicked() {
  Fndr *f = new Fndr();

  f->name = (ui->edit_name->text().isEmpty()) ? ui->edit_name->placeholderText()
                                              : ui->edit_name->text();
...
void FndrEdit::on_cancer_button_clicked() { close(); }
