...
OrgEdit::OrgEdit(QWidget *parent) : QDialog(parent), ui(new Ui::OrgEdit) {
  ui->setupUi(this);
}

OrgEdit::~OrgEdit() { delete ui; }
void OrgEdit::on_okay_button_clicked() {
  Org *org = new Org();

  org->label = (ui->country->text().isEmpty()) ? ui->country->placeholderText()
                                               : ui->country->text();
...
void OrgEdit::on_cancer_button_clicked() { close(); }
