...
public:
  ...
  virtual int insertOrg(const Org &, int userid) = 0;

  // courses
  virtual QVector<Fndr> getAllFndrs() = 0;
  virtual optional<Fndr> getFndrById(int, QString) = 0;
  ...
  virtual QVector<Org> getAllUserOrgs(int, QString, int) = 0;

  // links
  virtual QVector<Fndr> getAllOrgsFndrs(int, QString) = 0;
  virtual bool insertOrgFndr(int, int) = 0;
  virtual bool removeOrgFndr(int, int) = 0;
  virtual bool fndrOrgExec(int, int) = 0;

  //search
  virtual int countOrgs(int, QString) = 0;
};
