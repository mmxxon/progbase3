...
#define normal                                                                 \
  "background-color: rgb(181, 181, 181); border-radius:10px; color: rgb(255, " \
  "255, 255);"
#define wrong                                                                  \
  "color: rgb(255, 255, 255); border-radius:10px; background-color: rgb(255, " \
  "62, 81);"
...
void UserEdit::closeEvent(QCloseEvent *) { emit disableToggle(); }
...
void UserEdit::on_cancer_button_clicked() { this->close(); }

void UserEdit::on_okay_button_clicked() {
  QString l(ui->login->text()), cp(ui->cur_password->text()),
      p(ui->password->text()), c(ui->confirm->text());
  ...
  if (ui->pass_label->styleSheet() == wrong ||
      ui->confirm_label->styleSheet() == wrong ||
      ui->curp_label->styleSheet() == wrong)
    return;
  else if (hashPassword(cp) != user_->password) {
    QMessageBox::warning(this, "Err", "wrong current password",
                         QMessageBox::Ok);
    return;
  } else if (p != c) {
    ui->pass_label->setStyleSheet(wrong);
    ui->confirm_label->setStyleSheet(wrong);
    return;
  }
